...
{
    public partial class FrmUcretsizIzin : Form
    {
        public FrmUcretsizIzin()
        {
            InitializeComponent();
        }
        ...
        void listele()
        {
            ...
            SqlDataAdapter da = new SqlDataAdapter("exec UcretsizIzin_Getir", bgl.baglanti());
            da.Fill(dt);
        ...
        private void FrmUcretsizIzin_Load(object sender, EventArgs e)
        {
            listele();
            temizle();
        }
        ...
        {
            if (TxtUcretsizId.Text == "")
            {
                if (TxtPersonelId.Text != "")
                {
                    SqlCommand komut = new SqlCommand("insert into Ucretsiz_Izin (Personel_ID,Bas_Tarih,Bit_Tarih,Sebep) values (@p1,@p2,@p3,@p4)", bgl.baglanti());
            ...
            bgl.baglanti().Close();
            MessageBox.Show("Ücretsiz izin bilgisi oluşturuldu", "Bilgi", MessageBoxButtons.OK, MessageBoxIcon.Information);
            listele();
            temizle();
                }
        ...
        private void gridView1_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            TxtUcretsizId.Text = dr["UcretsizIzin_ID"].ToString();
        ...
        private void BtnSil_Click(object sender, EventArgs e)
        {
            if (TxtUcretsizId.Text != "")
            {
                DialogResult result = MessageBox.Show("Ücretsiz İzin Bilgisini Silmeyi Onaylıyor Musunuz ?", "Uyarı", MessageBoxButtons.OKCancel);
                if (result == DialogResult.OK)
                {
                    try
                    {
                        SqlCommand komutsil = new SqlCommand("delete from Ucretsiz_Izin where UcretsizIzin_ID=@p1", bgl.baglanti());
                        komutsil.Parameters.AddWithValue("@p1", TxtUcretsizId.Text);
        ...
        private void BtnGuncelle_Click(object sender, EventArgs e)
        {
            if (TxtUcretsizId.Text != "")
            {
                SqlCommand komutguncelle = new SqlCommand("update Ucretsiz_Izin set Bas_Tarih=@p1, Bit_Tarih=@p2,Sebep=@p4 where UcretsizIzin_ID=@p3", bgl.baglanti());
                ...
                komutguncelle.Parameters.AddWithValue("@p3", TxtUcretsizId.Text);
